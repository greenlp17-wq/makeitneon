...
import {
  ArrowLeft, Star, ShoppingCart, Shield, Zap, Package, Ruler,
  Check, Sparkles, Sun, CloudRain,
} from 'lucide-react';
...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (!product) {
      navigate(`/${currentLang}/shop`, { replace: true });
    }
  ...
  const [selectedSizeId, setSelectedSizeId] = useState(() =>
    product?.availableSizes[1]?.id || product?.availableSizes[0]?.id || ''
  );
  ...
  const description = isDE ? product.description_de : product.description_en;
  ...
  const detailsContent = (
    <>
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-semibold">{isDE ? 'Im Lieferumfang' : 'Includes'}</h4>
        <div className="grid grid-cols-2 gap-2">
          {[
            { icon: Zap, text: isDE ? 'Netzteil' : 'Power adapter' },
            { icon: Package, text: isDE ? 'Wandmontage-Set' : 'Wall mounting kit' },
            { icon: Shield, text: isDE ? '2 Jahre Garantie' : '2 year warranty' },
            { icon: Sparkles, text: isDE ? 'Dimmer & Fernbedienung' : 'Dimmer & remote' },
          ].map(({ icon: Icon, text }) => (
      ...
      {/* Specifications */}
      <div className="space-y-3">
        <h4 className="text-sm font-semibold">{isDE ? 'Spezifikationen' : 'Specifications'}</h4>
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          {[
            [isDE ? 'Material' : 'Material', isDE ? 'LED Neon Flex auf Acryl' : 'LED Neon Flex on Acrylic'],
            [isDE ? 'Leistung' : 'Wattage', '12V DC, 5-15W'],
            [isDE ? 'Lebensdauer' : 'Lifespan', '50,000+ hours'],
            ['IP', isOutdoor ? 'IP65 (Waterproof)' : 'IP20 (Indoor)'],
            [isDE ? 'Garantie' : 'Warranty', isDE ? '2 Jahre' : '2 Years'],
            [isDE ? 'Lieferzeit' : 'Delivery', isDE ? '5-7 Werktage' : '5-7 business days'],
          ].map(([label, value]) => (
  ...
  return (
    <>
      <SEO
        title={`${name} — LED Neon Sign | Make It Neon`}
        ...
        jsonLd={[
          ...
          createBreadcrumbSchema([
            ...
            { name: isDE ? 'Shop' : 'Shop', url: `/${currentLang}/shop` },
            { name, url: `/${currentLang}/shop/${product.slug}` },
          ]),
        ]}
      />
    <section className="section-padding" id="product-page">
      <div className="container-tight">
        ...
        >
          ...
          {isDE ? 'Zurück zum Shop' : 'Back to Shop'}
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="space-y-4">
            <div
              className="relative aspect-[4/3] rounded-3xl overflow-hidden bg-[#0A0A12] flex items-center justify-center"
              id="product-preview"
            >
              {product.image ? (
                ...
                <>
                  ...
                  {/* Neon text */}
                  <span
                    className="relative z-10 text-center px-8 leading-snug select-none"
                    style={{
              ...
              <div className="absolute top-4 left-4 flex gap-2 z-20">
                {product.bestSeller && (
                  <Badge className="bg-neon-warm text-black text-[10px] font-bold uppercase tracking-wider border-none">
                    Best Seller
                  </Badge>
                )}
                {product.isNew && (
                  <Badge className="bg-neon-green text-black text-[10px] font-bold uppercase tracking-wider border-none">
                    New
                  </Badge>
                )}
              </div>
              ...
              {/* Size overlay */}
              {selectedSize && (
                <div className="absolute bottom-4 right-4 z-20 bg-black/60 backdrop-blur-sm px-3 py-1.5 rounded-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">
                ({product.reviewCount} {isDE ? 'Bewertungen' : 'reviews'})
              </span>
            </div>
          ...
          <div className="space-y-6">
                    ...
                    <button
                      ...
                      className={`w-10 h-10 rounded-full transition-all duration-200 flex items-center justify-center
                        ${isActive
                          ...
                          : 'hover:scale-110 border-2 border-black/10'
                        }`}
                    ...
                    >
                      {isActive && (
                        <Check className="w-4 h-4" style={{
                          color: ['white', 'warm-white', 'yellow', 'lemon', 'green'].includes(colorId)
                            ? '#000' : '#fff'
                        }} />
                      )}
                    </button>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <div>
                    <div className="text-sm font-semibold">{isDE ? 'Innenbereich' : 'Indoor'}</div>
                    <div className={`text-xs ${!isOutdoor ? 'text-background/70' : 'text-muted-foreground'}`}>
                      {isDE ? 'Inklusive' : 'Included'}
                    </div>
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div>
                    <div className="text-sm font-semibold">{isDE ? 'Außenbereich' : 'Outdoor'}</div>
                    <div className={`text-xs ${isOutdoor ? 'text-background/70' : 'text-muted-foreground'}`}>
                      +40% (IP65)
                    </div>
                  </div>
            ...
            {/* Quantity */}
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold">{isDE ? 'Anzahl' : 'Quantity'}</h4>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-9 h-9 rounded-lg border border-border flex items-center justify-center 
                             hover:bg-muted transition-colors text-lg font-medium"
                  aria-label="Decrease"
                >
                  −
                </button>
            ...
            <div className="glass p-6 rounded-2xl space-y-4">
              <div className="flex items-end justify-between">
                <div>
                  <span className="text-sm text-muted-foreground">{isDE ? 'Gesamt' : 'Total'}</span>
                  <div className="text-3xl font-heading font-bold">{totalPrice} CHF</div>
                </div>
                {isOutdoor && (
                  <span className="text-xs text-muted-foreground">
                    {isDE ? 'inkl. Outdoor-Schutz' : 'incl. outdoor protection'}
                  </span>
                )}
              </div>
              ...
              <Button
                ...
                className={`w-full h-12 text-base font-heading font-semibold transition-all duration-300 ${
                  addedFeedback
                    ? 'bg-neon-green text-black shadow-[0_0_16px_rgba(57,255,20,0.3)]'
                    : 'bg-neon-pink hover:bg-neon-pink/90 text-white shadow-[0_0_16px_rgba(255,45,120,0.3)] hover:shadow-[0_0_24px_rgba(255,45,120,0.5)]'
                }`}
                id="add-to-cart"
              >
                {addedFeedback ? (
                  <><Check className="w-5 h-5 mr-2" />{isDE ? 'Hinzugefügt!' : 'Added!'}</>
                ) : (
                  <><ShoppingCart className="w-5 h-5 mr-2" />{isDE ? 'In den Warenkorb' : 'Add to Cart'}</>
                )}
              </Button>

              <p className="text-xs text-center text-muted-foreground">
                ✓ {isDE ? 'Kostenloses Design-Mockup' : 'Free design mockup'} &nbsp;•&nbsp;
                ✓ {isDE ? '2 Jahre Garantie' : '2 year warranty'} &nbsp;•&nbsp;
                ✓ {isDE ? 'Handgefertigt in Zürich' : 'Handcrafted in Zurich'}
              </p>
            </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-20">
            <h2 className="text-2xl mb-8 text-center">
              {isDE ? 'Ähnliche Schilder' : 'You Might Also Like'}
            </h2>
