...
export default function BlogPage() {
  ...
  return <>
      <SEO title={t('pages.blog.title')} description={t('pages.blog.description')} jsonLd={createBreadcrumbSchema([{
      name: 'Home',
      url: `/${currentLang}`
    }, {
      name: 'Blog',
      ...
      <section className="section-padding" id="blog-page">
        <div className="container-tight">
          ...
          <ScrollReveal direction="up" className="text-center mb-16">
            ...
            <h1 className="mb-6">
              {isUK ? <>Неоновий <span className="gradient-neon-text">Блог</span></> : isDE ? <>Neonschild-<span className="gradient-neon-text">Blog</span></> : <>Neon Sign <span className="gradient-neon-text">Blog</span></>}
            </h1>
          ...
          <ScrollReveal direction="up" delay={0.1}>
            <Link to={`/${currentLang}/blog/${featured.slug}`} className="block glass rounded-3xl overflow-hidden mb-12 group hover:shadow-xl transition-all duration-300" id="blog-featured">
              <div className="grid md:grid-cols-2 gap-0">
                {/* Image */}
                <div className="relative aspect-[16/10] md:aspect-auto flex items-center justify-center overflow-hidden bg-stone-900">
                  <img src={featured.image} alt={isUK ? featured.title.uk || featured.title.en : isDE ? featured.title.de : featured.title.en} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                ...
                <div className="p-6 md:p-8 lg:p-10 flex flex-col justify-center">
                  <div className="flex items-center gap-3 mb-3 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1">
                      ...
                      {new Date(featured.datePublished).toLocaleDateString(isUK ? 'uk-UA' : isDE ? 'de-CH' : 'en-US', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
                  ...
                  <h2 className="text-xl md:text-2xl font-heading font-bold mb-3 group-hover:text-neon-pink transition-colors">
                    {isUK ? featured.title.uk || featured.title.en : isDE ? featured.title.de : featured.title.en}
                  </h2>

                  <p className="text-muted-foreground text-sm leading-relaxed mb-5 line-clamp-3">
                    {isUK ? featured.metaDescription.uk || featured.metaDescription.en : isDE ? featured.metaDescription.de : featured.metaDescription.en}
                  </p>
          ...
          {/* Article Grid */}
          <StaggerGroup childSelector=".blog-card" staggerAmount={0.1} direction="up" distance={30} className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6 mb-16">
            {rest.map(article => <Link key={article.slug} to={`/${currentLang}/blog/${article.slug}`} className="blog-card glass rounded-2xl overflow-hidden group hover:shadow-lg transition-all duration-300" id={`blog-card-${article.slug}`}>
                {/* Real Image */}
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-heading font-bold text-base mb-2 group-hover:text-neon-pink transition-colors line-clamp-2">
                    {isUK ? article.title.uk || article.title.en : isDE ? article.title.de : article.title.en}
                  </h3>

                  <p className="text-muted-foreground text-sm line-clamp-2 mb-3">
                    {isUK ? article.metaDescription.uk || article.metaDescription.en : isDE ? article.metaDescription.de : article.metaDescription.en}
                  </p>
