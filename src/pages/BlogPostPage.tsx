...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    if (!article) {
      navigate(`/${currentLang}/blog`, { replace: true });
    }
  ...
  const title = isDE ? article.title.de : article.title.en;
  const description = isDE ? article.metaDescription.de : article.metaDescription.en;
  const content = isDE ? article.content.de : article.content.en;

  // Simple markdown-to-HTML conversion
  const renderMarkdown = (md: string) => {
    let html = md
      // Headers
      .replace(/^### (.+)$/gm, '<h3 class="text-lg font-heading font-bold mt-8 mb-3">$1</h3>')
      .replace(/^## (.+)$/gm, '<h2 class="text-xl md:text-2xl font-heading font-bold mt-10 mb-4">$1</h2>')
      // Bold
    ...
    html = html.replace(/(<li[^>]*>.*?<\/li>\n?)+/g, (match) => {
      ...
      return isOrdered
        ? `<ol class="space-y-1 mb-4">${match}</ol>`
        : `<ul class="space-y-1 mb-4">${match}</ul>`;
    });

    // Wrap <tr> in <table>
    html = html.replace(/(<tr>.*?<\/tr>\s*)+/g, (match) =>
      `<table class="w-full border-collapse border border-border rounded-lg overflow-hidden mb-6">${match}</table>`
    );
      ...
      <SEO
        ...
        jsonLd={[
          ...
          createBreadcrumbSchema([
            ...
            { name: title, url: `/${currentLang}/blog/${article.slug}` },
          ]),
        ]}
      />
                ...
                {related.map((rel) => (
                  ...
                  >
                    <div className="aspect-[16/9] flex items-center justify-center overflow-hidden relative bg-stone-900">
                      <img 
                        src={rel.image} 
                        alt={isDE ? rel.title.de : rel.title.en} 
                    ...
                    <div className="p-4">
                      <h3 className="font-heading font-bold text-sm mb-1 group-hover:text-neon-pink transition-colors line-clamp-2">
                        {isDE ? rel.title.de : rel.title.en}
                      </h3>
